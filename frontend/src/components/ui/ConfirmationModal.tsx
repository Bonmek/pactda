...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  function extractEscrowProps(child: any) {
    if (child && typeof child === 'object' && child.props) {
      if (typeof child.props.escrowAmount === 'number')
        escrowAmount = child.props.escrowAmount
      if (
        child.props.escrowAction === 'refund' ||
        child.props.escrowAction === 'fund'
      )
        escrowAction = child.props.escrowAction
    }
  }
  ...
  if (escrowAmount && escrowAction === 'refund') {
    escrowHighlight = (
      <span className="ml-2 text-green-400 font-bold animate-pulse">
        +{escrowAmount} SUI
      </span>
    )
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <h2 className="text-2xl font-semibold mb-4 text-center bg-clip-text text-transparent bg-gradient-to-r from-sky-400 to-cyan-300">
              {title}
            </h2>
            ...
            <div className="mb-4 p-3 bg-slate-800/40 rounded-lg border border-slate-700">
              ...
              {currentBalance !== undefined && currentBalance !== null ? (
                <div className="text-base text-sky-300 font-semibold flex items-center">
                  {currentBalance} SUI {escrowHighlight}
                </div>
              ) : (
