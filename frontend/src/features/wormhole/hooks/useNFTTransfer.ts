...
export function useNFTTransfer({
  ...
  const transferNFT = async ({
    ...
    try {
      ...
      const nftAddress = getNFTCollectionBySymbolAndChain(
        collectionSymbol,
        sourceChain,
      )
      if (!nftAddress) {
        throw new Error(
          `NFT collection ${collectionSymbol} not supported on chain ${sourceChain}`,
        )
      }
      ...
      if (!recipientAddress) {
        if (
          targetChain === Number(import.meta.env.VITE_CHAIN_ID_SUI) &&
          suiAccount
        ) {
          recipientAddress = suiAccount.address
        } else if (
          targetChain === Number(import.meta.env.VITE_CHAIN_ID_ETH) &&
          isEthConnected &&
          ethAddress
        ) {
          ...
          throw new Error(
            'No recipient address provided and no wallet connected for target chain',
          )
        }
      }
      ...
      if (sourceChain === Number(import.meta.env.VITE_CHAIN_ID_ETH)) {
        ...
        result = await transferNFTFromSuiToEth(
          suiProvider,
          suiWallet,
          transferParams,
        )
