...
import { CHAINS, SUPPORTED_TOKENS, PACTDA_CONFIG } from '../config';
import { TransferStatus } from '../types';

interface TokenBridgeFormProps {
  defaultSourceChain?: number;
  defaultTargetChain?: number;
}

export const TokenBridgeForm: React.FC<TokenBridgeFormProps> = ({
  ...
  const [selectedToken, setSelectedToken] = useState(SUPPORTED_TOKENS[0]?.symbol || 'USDC');
  const [amount, setAmount] = useState('');
  const [recipient, setRecipient] = useState('');
  const [txHash, setTxHash] = useState('');

  // Get token transfer hook
  const { transferToken, completeTransfer, status, resetStatus } = useTokenTransfer({
    sourceChain,
    targetChain,
  });
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const amountInSmallestUnit = (parseFloat(amount) * 10 ** decimals).toString();
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6 bg-gray-800 rounded-xl border border-gray-700 shadow-xl">
      <h2 className="text-2xl font-bold text-center mb-6">PactDA Cross-Chain Token Bridge</h2>
      <p className="text-gray-400 text-center mb-8">
        Transfer tokens between Ethereum and Sui networks securely using Wormhole
      </p>
        ...
        <div>
          ...
          <div className="relative">
            <input 
              ...
              min="0"
              step="0.000001"
              className="w-full bg-gray-700 border border-gray-600 rounded-lg p-3 pr-16 text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
      ...
      <div className="mt-8 pt-6 border-t border-gray-700">
        ...
        <p className="text-sm text-gray-400 mb-4">
          If you have a pending transfer that needs to be completed on the target chain, enter the source transaction hash below.
        </p>
