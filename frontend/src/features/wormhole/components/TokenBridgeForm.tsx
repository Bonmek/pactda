...
export const TokenBridgeForm: React.FC<TokenBridgeFormProps> = ({
  ...
  const [selectedToken, setSelectedToken] = useState(
    SUPPORTED_TOKENS[0]?.symbol || 'USDC',
  )
  ...
  const { transferToken, completeTransfer, status, resetStatus } =
    useTokenTransfer({
      sourceChain,
      targetChain,
    })
  ...
  const StatusDisplay = ({ status }: { status: TransferStatus }) => (
    ...
    >
      ...
      {status.txHash && (
        <p className="text-sm">
          TX Hash:{' '}
          <span className="font-mono text-xs break-all">{status.txHash}</span>
        </p>
      )}
      {status.error && (
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6 bg-gray-800 rounded-xl border border-gray-700 shadow-xl">
      <h2 className="text-2xl font-bold text-center mb-6">
        PactDA Cross-Chain Token Bridge
      </h2>
      <p className="text-gray-400 text-center mb-8">
        Transfer tokens between Ethereum and Sui networks securely using
        Wormhole
      </p>
      ...
      <form onSubmit={handleTransfer} className="space-y-6">
        ...
        >
          {status.status === 'preparing' || status.status === 'signing'
            ? 'Preparing Transaction...'
            : status.status === 'sending' || status.status === 'confirming'
              ? 'Processing...'
              : 'Transfer Tokens'}
        </button>
      </form>
      ...
      <div className="mt-8 pt-6 border-t border-gray-700">
        ...
        <p className="text-sm text-gray-400 mb-4">
          If you have a pending transfer that needs to be completed on the
          target chain, enter the source transaction hash below.
        </p>
