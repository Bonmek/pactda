  ...
  const executeTransaction = useCallback(
    async (transaction: Transaction) => {
      try {
        ...
        if (zkloginAddress) {
          console.log(
            'Executing transaction with zkLogin authentication, address:',
            zkloginAddress,
          )
          ...
          try {
            return await signAndExecuteTransactionWithZkLogin(
              transaction,
              suiClient,
            )
          } catch (zkError) {
            ...
            if (zkError instanceof Error) {
              ...
              if (
                errorMsg.includes('invalid user signature') ||
                errorMsg.includes('signature is not valid')
              ) {
                console.error('zkLogin authentication signature error detected')
                throw new Error(
                  'Authentication signature invalid. Please log out and login again using zkLogin.',
                )
              }
              // Salt-related errors
                ...
                throw new Error(
                  'Authentication error: Problem with user identity data. Please log out and login again.',
                )
              }
              // JWT or token expiration
                ...
                throw new Error(
                  'Your authentication session has expired. Please log out and login again.',
                )
              }
              // ZK Proof errors
              else if (
                errorMsg.includes('proof') ||
                errorMsg.includes('groth16')
              ) {
                console.error('zkLogin proof verification error detected')
                throw new Error(
                  'Authentication verification failed. Please try again or use a different login method.',
                )
              }
            }
