...
import {
  ...
  buildFundEscrowTx,
  buildRefundEscrowTx,
  buildCreateContractWithMilestonesTx,
...
export const submitContract = async (
  ...
  try {
    ...
    toast.error(
      `Error submitting contract: ${error instanceof Error ? error.message : 'Unknown error'}`,
    )
  }
}
...
export const fundEscrow = async (
  ...
  const txb = await buildFundEscrowTx(
    contract.objectId,
    amount,
    address,
    suiClient,
...
export function milestonesChanged(
  oldMilestones: any[],
  newMilestones: any[],
): boolean {
  ...
  for (let i = 0; i < oldMilestones.length; i++) {
    ...
    if (a.description !== b.description || String(a.value) !== String(b.value))
      return true
  }
  return false
}
    ...
    if (oldMilestones && milestonesChanged(oldMilestones, milestones)) {
      txb = buildUpdateContractTx(contractData.contractId!, {
        ...
        startDate: contractData.startDate
          ? new Date(contractData.startDate * 1000)
          : undefined,
        endDate: contractData.endDate
          ? new Date(contractData.endDate * 1000)
          : undefined,
