...
export const signAndExecuteTransactionWithZkLogin = async (
  ...
  try {
    ...
    console.log("JWT decoded - issuer:", decodedJwt.iss, "subject type:", typeof decodedJwt.sub)
    ...
    const { ephemeralKeypair, randomness, maxEpoch } = restoreEphemeralKeyAndRandomness()
    console.log("Restored ephemeral key and randomness, maxEpoch:", maxEpoch)
    
    // Get the stored salt from server
    const salt = await fetchSaltFromSessionOrServer(decodedJwt.iss, decodedJwt.sub)
    ...
    if (!verifyZkLoginAddress(idToken, salt, zkLoginAddress)) {
      console.error("zkLogin address verification failed - session may be corrupted")
      throw new Error('Authentication session is invalid. Please log out and login again.')
    }
    
    // Set the sender address to the zkLogin address - IMPORTANT: Must use setSender explicitly
    console.log(`Setting transaction sender to zkLogin address: ${zkLoginAddress}`)
    ...
    try {
      ...
      throw new Error(`Failed to get zkLogin proof: ${error instanceof Error ? error.message : String(error)}`)
    }
    ...
    try {
      ...
      throw new Error(`Failed to create zkLogin signature: ${error instanceof Error ? error.message : String(error)}`)
    }
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'message' in error) {
        ...
        if (errorMessage.includes("Invalid user signature")) {
          console.error("This is likely an issue with the zkLogin authentication. Check the salt, JWT, or signature generation.")
        }
      }
 ...
 * might be corrupted.
 */
const verifyZkLoginAddress = (jwt: string, salt: string, storedAddress: string): boolean => {
  try {
 ...
 */
const fetchSaltFromSessionOrServer = async (issuer: string, subject: string): Promise<string> => {
  ...
  try {
    if (!BACKEND_API_URL) {
      throw new Error('Backend API URL not configured. Set VITE_BACKEND_API_URL in environment.')
    }
    
    console.log(`Fetching salt from ${BACKEND_API_URL}/salt for subject: ${subject.substring(0, 5)}...`)
    ...
    throw new Error(`Failed to fetch salt: ${error instanceof Error ? error.message : String(error)}`)
  }
}
