...
export function useContractActions({ contract, escrow, setContract, setShowFundModal, setFunding, walletBalance, setShowRefundModal }: UseContractActionsParams) {
  ...
  const handleSubmitContract = async () => {
    if (!contract || !address) return;
    await submitContract(contract, signAndExecuteTransaction, suiClient, async () => setContract(await getContractDetail(suiClient, contract.objectId)));
  };
  ...
  const showFundEscrowButton = (!contract?.escrowId || isRefunded)  && contract?.status === 2;
  const showRefundEscrowButton = !!contract?.escrowId && isFunded && !isRefunded; //contract = active
