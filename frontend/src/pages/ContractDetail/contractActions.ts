...
import {
  ...
  fundEscrow,
  refundEscrow,
} from '@/service/PactdaContractActions'
...
export function useContractActions({
  ...
  const handleSignContract = async () => {
    ...
    await signContract(
      ...
      suiClient,
      async () =>
        setContract(await getContractDetail(suiClient, contract.objectId)),
    )
  }
  ...
  const showFundEscrowButton =
    (!contract?.escrowId || isRefunded) && contract?.status === 2
